...
{
	int opcaoCurta, auxiliar;
	char *opcoesCurtas = "dDhHsSuU";
	tipoErros resultado;
	struct option opcoesLongas [5] = { {"dos", 0, NULL, 'd'}, {"show", 0, NULL, 's'}, 
		{"help", 0, NULL, 'h'},	{"unix", 0, NULL, 'u'}, {NULL, 0, NULL, 0} };
	...
	while ((opcaoCurta = getopt_long (argc, argv, opcoesCurtas, opcoesLongas, &auxiliar) ) != -1)
	{
		switch (opcaoCurta)
		{
			...
			{
				printf ("O usuario deve digitar o nome do primeiro arquivo!.\n");
				exit (NUMERO_DE_ARGUMENTOS_INVALIDO);
			}
			if (strlen(nomeArquivo) > 0)
			{
      		...
      		case 'h':
      		case 'H':
       		printf ("Opcao selecionada: AJUDA \n\n");	
			printf ("Comandos:\n\n");
			...
			case 's':
			case 'S':
			printf ("Opcao selecionada: EXIBIR\n");
			if (argc > 2)
			{
				if (strlen(argv[optind]) > 0 && strlen(argv[optind]) < COMPRIMENTO_MAXIMO_NOME_ARQUIVO)
				{
